...
async def run_audio_only_pipeline(audio_path: str, output_dir: str, on_progress) -> dict:
    ...
    auto_voice_map = await dubber.extract_character_voice_samples(audio_path, segments, output_dir)

    total = len(segments)
    for i, seg in enumerate(segments):
        ref = auto_voice_map.get(seg.get('speaker_id'))
    ...
    try:
        ...
        if is_video:
            result = await dubber.process_khmer_dubbing(
                input_path, extracted_audio_path, job_dir, {}, on_progress=on_progress
            )
            job['mediaType'] = 'video'
            job['outputUrl'] = f"/media/{job_id}/{result['outputVideoFilename']}"
            job['script'] = result.get('khmerScript', '')
        else:
            result = await run_audio_only_pipeline(extracted_audio_path, job_dir, on_progress)
            job['mediaType'] = 'audio'
...
@app.post('/api/dub')
async def start_dub(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    background_tasks.add_task(run_job, job_id, input_path)
    return {'jobId': job_id}
